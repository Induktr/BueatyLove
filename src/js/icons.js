...
export const icons = {
    ...
    // Service icons
    services: {
        brows: `<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
        </svg>`,
        lashes: `<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
        </svg>`,
        makeup: `<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    actions: {
        ...
        call: `<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
        </svg>`,
        email: `<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    social: {
        ...
        telegram: `<svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    ui: {
        ...
        theme: `<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function insertIcon(iconPath, className = '') {
    const [category, name] = iconPath.split('.')
    const icon = icons[category]?.[name]
    if (!icon) return ''
    
    // If className is provided, inject it into the svg
    if (className) {
        return icon.replace('class="w-6 h-6"', `class="${className}"`)
    }
    return icon
}
...
export function initIcons() {
    document.querySelectorAll('[data-icon]').forEach(element => {
        ...
        element.innerHTML = insertIcon(iconPath, className)
    })
}
